...
TextMind 基于 [Wails v2](https://wails.io/)（Go）+ [Vue 3](https://vuejs.org/) + [Monaco Editor](https://microsoft.github.io/monaco-editor/) 构建，把多标签编辑、文件浏览器、常用文本处理工具，以及兼l容 OpenAI 协议的 AI 对话/选区改写整合到一个原生应用里。所有数据本地保存，不依赖任何云服务。
...
* **命令行/拖拽打开**：把文件作为参数传给 `TextMind.exe` 即可在新会话中直接打开。npm install
...
| **提取（Extract）**   | 支持三种模式：· `filter` 按关键字 / 正则保留或反选整行· `capture` 按正则捕获组提取子串· `block` 按起止正则截取多行块 |
...
另有以下默认未绑定的动作，可在快捷键设置中按需挂载：打开文件、打开文件夹、自动识别语言、提取文本、去重、保留单次出现项、保留重复项、转 IN 列表、格式化 JSON、压缩 JSON、插入选中文本到 AI 对话框。
